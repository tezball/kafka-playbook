...
if [ "$SKIP_TESTS" = false ]; then
  ...
  run_tests "query-service" "query-service"
  echo ""
else
...
for name in kafka catalog-producer query-service akhq; do
...
# Wait for services
wait_for_health "Catalog Producer" "http://localhost:8080/actuator/health" 180
wait_for_health "Query Service" "http://localhost:8081/actuator/health" 180
...
open_browser "http://localhost:8888/ui/kafka-playbook/topic/product-commands/data?sort=NEWEST&partition=All"
...
echo "  Catalog Producer: http://localhost:8080"
...
echo "    Live Messages:   http://localhost:8888/ui/kafka-playbook/topic/product-commands/data?sort=NEWEST&partition=All"
echo "    Consumer Groups: http://localhost:8888/ui/kafka-playbook/group"
echo ""
echo "  Product commands are being auto-generated every 10 seconds."
echo ""
echo "  Try:"
echo "    docker compose logs -f query-service               # watch materializer"
echo "    curl -s localhost:8081/api/products | jq            # query the view"
echo "    curl -s localhost:8080/api/products/sample          # send a manual command"
echo ""
echo "  See LESSON.md for the full exercise guide."
echo "=========================================="
echo ""

# Tail producer and query-service logs so the user sees events flowing live
echo ">>> Tailing catalog-producer & query-service logs (Ctrl+C to stop)..."
docker compose logs -f --tail=50 catalog-producer query-service
