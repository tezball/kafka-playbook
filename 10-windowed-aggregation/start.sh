...
echo "=========================================="
echo "  Kafka Playbook — 10 Windowed Aggregation"
echo "=========================================="
echo ""
...
if [ "$SKIP_TESTS" = false ]; then
  ...
  run_tests "aggregator" "aggregator"
  echo ""
else
...
for name in kafka order-producer dashboard-aggregator akhq; do
...
wait_for_health "Aggregator" "http://localhost:8081/actuator/health" 180
...
open_browser "http://localhost:8888/ui/kafka-playbook/topic/category-counts/data?sort=NEWEST&partition=All"
...
echo "  Aggregator: http://localhost:8081"
echo "  AKHQ:       http://localhost:8888"
echo ""
echo "  AKHQ Views:"
echo "    Topics:          http://localhost:8888/ui/kafka-playbook/topic"
echo "    Dashboard Orders:http://localhost:8888/ui/kafka-playbook/topic/dashboard-orders/data?sort=NEWEST&partition=All"
echo "    Category Counts: http://localhost:8888/ui/kafka-playbook/topic/category-counts/data?sort=NEWEST&partition=All"
echo "    Consumer Groups: http://localhost:8888/ui/kafka-playbook/group"
echo ""
echo "  Orders are being auto-generated every 10 seconds."
echo ""
echo "  Try:"
echo "    docker compose logs -f aggregator            # watch windowed counts"
echo "    curl -s localhost:8081/api/dashboard/counts   # query live counts"
...
# Tail producer and aggregator logs so the user sees events flowing live
echo ">>> Tailing producer & aggregator logs (Ctrl+C to stop)..."
docker compose logs -f --tail=50 producer aggregator
