...
echo "=========================================="
echo "  Kafka Playbook — 13 Outbox Pattern"
echo "=========================================="
echo ""
...
if [ "$SKIP_TESTS" = false ]; then
  ...
  run_tests "order-service" "order-service"
  echo ""
else
...
for name in kafka postgres order-service notification-consumer akhq; do
...
# Wait for services
wait_for_health "Order Service" "http://localhost:8080/actuator/health" 180
wait_for_health "Notification Consumer" "http://localhost:8081/actuator/health" 180
...
open_browser "http://localhost:8888/ui/kafka-playbook/topic/order-events/data?sort=NEWEST&partition=All"
...
echo "    Live Messages:   http://localhost:8888/ui/kafka-playbook/topic/order-events/data?sort=NEWEST&partition=All"
echo "    Consumer Groups: http://localhost:8888/ui/kafka-playbook/group"
echo ""
echo "  Orders are being auto-generated every 10 seconds."
echo "  The outbox poller publishes events to Kafka every 2 seconds."
echo ""
echo "  Try:"
echo "    docker compose logs -f notification-consumer  # watch notifications"
echo "    docker compose logs -f order-service          # watch outbox flow"
echo "    curl -s localhost:8080/api/orders/sample       # send a manual order"
echo "    curl -s localhost:8080/api/orders              # list all orders"
echo ""
echo "  See LESSON.md for the full exercise guide."
echo "=========================================="
echo ""

# Tail order-service and notification-consumer logs so the user sees events flowing live
echo ">>> Tailing order-service & notification-consumer logs (Ctrl+C to stop)..."
docker compose logs -f --tail=50 order-service notification-consumer
